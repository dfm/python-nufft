...
class TestType1(unittest.TestCase):
    ...
    def _type_1_even(self, eps=1e-10):
        p2 = nufft1(self.x, self.y, len(self.f), direct=True)
        p1 = nufft1(self.x, self.y, len(self.f), eps=eps)
        err = np.sqrt(np.sum(np.abs(p1 - p2) ** 2) / np.sum(np.abs(p1)**2))
        self.assertTrue(err < eps, "Discrepancy between direct and fft function")
        self.assertEqual(len(nufft1freqs(len(f))), len(p1), "Wrong length of frequency array")

    def _type_1_odd(self, eps=1e-10):
        p2 = nufft1(self.x, self.y, len(self.f) + 1, direct=True)
        p1 = nufft1(self.x, self.y, len(self.f) + 1, eps=eps)
        err = np.sqrt(np.sum(np.abs(p1 - p2) ** 2) / np.sum(np.abs(p1)**2))
        self.assertTrue(err < eps, "Discrepancy between direct and fft function")
        self.assertEqual(len(nufft1freqs(len(f) + 1)), len(p1), "Wrong length of frequency array")

    def test_type_1_even_2(self):
        eps = 1e-2
        err = _type_1_even(self, eps)
        self.assertTrue(err < eps, "Discrepancy between direct and fft function")
